...
export class NetNode {
    ...
    public init(socket: ISocket, protocol: IProtocolHelper, networkTips: INetworkTips | null = null, execFunc: ExecuterFunc | null = null) {
        engine.logger.log(LOG_TAG.GAME,`网络初始化`);
        this._socket = socket;
    ...
    protected onConnected(event: any) {
        engine.logger.log(LOG_TAG.GAME,"网络已连接")
        ...
        engine.logger.log(LOG_TAG.GAME,`网络已连接当前状态为【${NetNodeStateStrs[this._state]}】`);
    }

    /** 连接验证成功，进入工作状态 */
    protected onChecked() {
        engine.logger.log(LOG_TAG.GAME,"连接验证成功，进入工作状态");
        ...
        if (requests.length > 0) {
            engine.logger.log(LOG_TAG.GAME,`请求【${this._requests.length}】个待发送的信息`);
    ...
    protected onMessage(msg: any): void {
        // engine.logger.log(LOG_TAG.GAME,`接受消息状态为【${NetNodeStateStrs[this._state]}】`);
        ...
        if (this._requests.length > 0) {
            ...
            if (this._requests.length == 0) {
                ...
                engine.logger.log(LOG_TAG.GAME,`请求队列中还有【${this._requests.length}】个请求在等待`);
            }
        }
        ...
        if (null != listeners) {
            for (const rsp of listeners) {
                engine.logger.log(LOG_TAG.GAME,`触发监听命令【${rspCmd}】的回调`);
    ...
    protected onClosed(event: any) {
        ...
        if (this._disconnectCallback && !this._disconnectCallback()) {
            engine.logger.log(LOG_TAG.GAME,`断开连接`);
            return;
        }
    ...
    public send(buf: NetData, force: boolean = false): number {
        if (this._state == NetNodeState.Working || force) {
            ...
            engine.logger.log(LOG_TAG.GAME,`当前状态为【${NetNodeStateStrs[this._state]}】,繁忙并缓冲发送数据`);
            return 0;
        }
        else {
    ...
    public requestUnique(reqProtocol: IRequestProtocol, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false): boolean {
        ...
        for (let i = 0; i < this._requests.length; ++i) {
            if (this._requests[i].rspCmd == rspCmd) {
                engine.logger.log(LOG_TAG.GAME,`命令【${rspCmd}】重复请求`);
                return false;
            }
        }
    ...
    private base_request(reqProtocol: IRequestProtocol, rspCmd: string, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        ...
        engine.logger.log(LOG_TAG.GAME,`队列命令为【${rspCmd}】的请求，等待请求数据的回调`);
        // 进入发送缓存列表
        this._requests.push({
    ...
    protected resetHearbeatTimer() {
        ...
        this._keepAliveTimer = setTimeout(() => {
            engine.logger.log(LOG_TAG.GAME,"网络节点保持活跃发送心跳信息");
